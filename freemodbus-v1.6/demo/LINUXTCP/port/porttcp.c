...
BOOL
xMBTCPPortInit( USHORT usTCPPort, CHAR *SrvAddr)
{
    USHORT          usPort;
    ...
    if (SrvAddr){
	serveraddr.sin_addr.s_addr = inet_addr(SrvAddr);
    } else {
	...
	/* TCPPort Init success */
	//ipaddr=inet_ntoa(serveraddr.sin_addr);
	fprintf( stderr,"\r\n INADDR_ANY is %s",htonl(INADDR_ANY));
	fprintf( stderr,"\r\n Create socket success xListenSocket= 0x%x ",xListenSocket);
	fprintf( stderr,"\r\n listen serveraddr %d on port %d ",inet_ntoa(serveraddr.sin_addr),usPort);	
	fprintf( stderr,"\r\n listen result = %d \r\n",rv);
